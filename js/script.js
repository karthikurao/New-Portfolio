...
function checkScrollPosition() {
    // Initialize contact dialog to be hidden
    document.getElementById('contact-me-pane').classList.add('contact-me-inactive');
    document.getElementById('contact-me-pane').classList.remove('contact-me-active');
}

function projectPaused() {
    swal("Project Paused", "This project is currently paused for further development. Check back later!", "info");
}

function projectRetired() {
    swal("Project Retired", "This project has been retired. It was a good run!", "info");
}

function iterateClickCount() {
    // Easter egg for clicking the profile image
    let clicks = localStorage.getItem('zach-image-clicks') || 0;
    clicks = parseInt(clicks) + 1;
    localStorage.setItem('zach-image-clicks', clicks);
    
    if(clicks === 10) {
        swal("Easter Egg Found!", "You've clicked my photo 10 times. You're persistent!", "success");
    }
}
